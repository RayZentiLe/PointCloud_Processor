...
if VTK_AVAILABLE:
    ...
    class CrossSectionPreviewWidget(QWidget):
        ...
        def set_preview_data(self, layer_points, plane_origin, plane_normal_xy, thickness):
        ...
        def _build_scene(self):
            preview_points = self._compute_preview_points()
            if len(preview_points) > 0:
                actor = self._make_points_actor(preview_points, color=(0.9, 0.4, 0.1), point_size=4)
...
class CrossSectionPanel(QWidget):
    ...
    def _on_define_normal(self):
        ...
        self._status_label.setText(
            "Normal selection mode: click the first point, move the mouse to preview, then click the second point. Right-click cancels."
        )
        self._update_define_normal_button()
